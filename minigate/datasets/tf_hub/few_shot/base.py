...
from minigate.datasets.data_utils import (
...
class FewShotClassificationDatasetTFDS(Dataset):
    def __init__(
        ...
        self.subsets = []
        log.info(
            f"Loading split {split_name} dataset {dataset_name} from {dataset_root}"
        )
        ...
        for subset_name in subset_split_name_list:
            ...
            self.subsets.append(list(subset.cache().as_numpy_iterator()))
